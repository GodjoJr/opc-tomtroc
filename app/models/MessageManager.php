...
class MessageManager extends ModelManager
{
    ...
    public function getAllMessagesFromAConversation($receiver_id, $sender_id)
    {
        $sql = "SELECT * FROM messages WHERE (m_sender_id = :receiver_id AND m_receiver_id = :sender_id) OR (m_sender_id = :sender_id AND m_receiver_id = :receiver_id) ORDER BY m_created_at ASC";
